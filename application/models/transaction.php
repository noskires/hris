...
class Transaction extends CI_Model
{
	...
	public function get_count_per_organization($org_code)
	{
		...
		COUNT( * ) AS count, SUM( TIMESTAMPDIFF( YEAR, date_birth, CURDATE( ) ) ) / COUNT( * ) AS average, SUM( TIMESTAMPDIFF( YEAR, date_birth, CURDATE( ) ) ) as sum_age 
					FROM emp_data 
	...
	public function get_count_per_organization_per_subgroup($org_code)
	{
		$org_code = trim($org_code); 
		
		$str1 = "SELECT COUNT( emp_id ) AS cnt, 
                         emp_data.emp_subgroup, 
		         COUNT( * ) AS count, 
                         SUM( TIMESTAMPDIFF( YEAR, date_birth, CURDATE( ) ) ) / COUNT( * ) AS average, 
		         SUM( TIMESTAMPDIFF( YEAR, date_birth, CURDATE( ) ) ) as sum_age, 
                         SUM( datediff(curdate(),date_hired) / 365.25) / COUNT( * ) AS average_los, 
		         SUM( datediff(curdate(),date_hired) / 365.25) as sum_los 
					FROM emp_data 
					WHERE CASE WHEN emp_data.group_code !=99999999 
					THEN emp_data.group_code = '$org_code' 
					OR emp_data.sector_code = '$org_code' 
		...
		$str2 = "SELECT COUNT( emp_id ) AS cnt, emp_data.emp_subgroup, 
		COUNT( * ) AS count, 
                SUM( TIMESTAMPDIFF( YEAR, date_birth, CURDATE( ) ) ) / COUNT( * ) AS average, 
		SUM( TIMESTAMPDIFF( YEAR, date_birth, CURDATE( ) ) ) as sum_age,
                SUM( datediff(curdate(),date_hired) / 365.25) / COUNT( * ) AS average_los, 
	...
	public function get_count_per_organization_per_age($org_code)
	{
		$org_code = trim($org_code); 
		
		$str1 = "SELECT COUNT( emp_id ) AS cnt, emp_data.emp_subgroup, 
		CASE 
					...
					OR emp_data.center_code = '$org_code' 
					OR emp_data.div_code = '$org_code' 
					END GROUP BY emp_subgroup, agegroup"; 
	...
	public function get_count_per_organization_per_tenure($org_code)
	{
		$org_code = trim($org_code); 
		...
		$str1 = "SELECT COUNT( emp_id ) AS cnt, emp_data.emp_subgroup, 
				...
				OR emp_data.center_code = '$org_code' 
				OR emp_data.div_code = '$org_code' 
				END GROUP BY emp_subgroup, los_group"; 
		...
		$str2 = "SELECT COUNT( emp_id ) AS cnt, emp_data.emp_subgroup, 
				...
				WHEN datediff(curdate(),date_hired) / 365.25 > 40 THEN '> 40' END AS los_group 
					FROM emp_data 
					WHERE emp_data.group_code =99999999  
					GROUP BY emp_subgroup, los_group";
	...
	public function get_count_hired_per_year()
	{
		//$str = "SELECT COUNT( * ) AS cnt, YEAR( date_hired ) AS year_hired
                ...
                $str = "SELECT COUNT( * ) AS cnt, year_hired 
					FROM all_emps
					WHERE year_hired >1999
					GROUP BY year_hired ";
...
// distribution by separation
	...
	{
		$str = "SELECT emp_subgroup, COUNT( * ) AS count, 
				SUM( TIMESTAMPDIFF( YEAR, date_birth, CURDATE( ) ) ) / COUNT( * ) AS average, 
				SUM( TIMESTAMPDIFF( YEAR, date_birth, CURDATE( ) ) ) as sum_age,
				SUM( datediff(curdate(),date_hired) / 365.25) / COUNT( * ) AS average_los, 
				SUM( datediff(curdate(),date_hired) / 365.25) as sum_los			
				FROM emp_data GROUP BY emp_subgroup";
...
public function get_average_per_gender()
	{
		$str = "SELECT sex, COUNT( * ) AS count, 
				SUM( TIMESTAMPDIFF( YEAR, date_birth, CURDATE( ) ) ) / COUNT( * ) AS average, 
				SUM( TIMESTAMPDIFF( YEAR, date_birth, CURDATE( ) ) ) as sum_age, 
				SUM( datediff(curdate(),date_hired) / 365.25) / COUNT( * ) AS average_los, 
				SUM( datediff(curdate(),date_hired) / 365.25) as sum_los
				FROM emp_data GROUP BY sex ";
...
public function get_average_per_pers_area()
	{
		$str = "SELECT personnel_area, COUNT( * ) AS count, 
				SUM( TIMESTAMPDIFF( YEAR, date_birth, CURDATE( ) ) ) / COUNT( * ) AS average, 
				SUM( TIMESTAMPDIFF( YEAR, date_birth, CURDATE( ) ) ) as sum_age,
				SUM( datediff(curdate(),date_hired) / 365.25) / COUNT( * ) AS average_los, 
				SUM( datediff(curdate(),date_hired) / 365.25) as sum_los 
				FROM emp_data GROUP BY personnel_area ";
...
//-------------------11-10-2016------------------------//
	public function authenticate($user_account, $pass_account)
	{
                date_default_timezone_set("Asia/Manila");

		$str = "SELECT user_account, pass_account, name FROM users 
				WHERE user_account= '$user_account' 
				AND pass_account = BINARY md5('$pass_account')";
		$query = $this->db->query($str); 
		$num_rows = $query->num_rows();
		$user_data = $query->row();
		$name = $user_data->name;
		...
		if($num_rows > 0)
		{
			...
			if($pass_account==="Password01")
			{
				echo "<script>alert('Your password is not secured, please change it immediately. Thank you!');</script>"; 
			...
			else
			{
				echo "<script>window.location='index';</script>"; 
                        ...
                        //update latest login and hits
			$today = date('Y-m-d H:i:s');
			$str = "UPDATE users SET last_login = '$today' , no_of_login = no_of_login + 1 WHERE user_account = '$user_account'";
			$query = $this->db->query($str); 
		}
		else
		{
	...
	public function change_the_pass($user_account, $curr_pass_account, $pass_account)
	{
		
		$str = "SELECT user_account, pass_account, name FROM users 
				WHERE user_account= '$user_account' 
				AND pass_account = BINARY md5('$curr_pass_account')";
		$query = $this->db->query($str); 
		$num_rows = $query->num_rows();
		$user_data = $query->row();
		$name = $user_data->name;
		if($num_rows > 0)
		{
			$pass_account = md5($pass_account);
			$str = "UPDATE users SET pass_account = '$pass_account' WHERE user_account = '$user_account'";
			$query = $this->db->query($str); 
			return "Success";
		}
		else
