...
// distribution by separation
	...
	{ 
		$str = "SELECT DISTINCT termination_reason FROM  `emp_data_separated` WHERE termination_reason NOT LIKE '%End of Contract%'    "; 
		$query = $this->db->query($str); 
		$num_rows = $query->num_rows();
		if($num_rows > 0)
		{
	...
	public function get_count_by_age_group_separated()
	{
		$str = "SELECT COUNT( * ) AS count, termination_reason, DATEDIFF( date_separated , date_hired ) / 365.25 AS los, 
					CASE 
					WHEN TIMESTAMPDIFF( YEAR, date_birth, date_separated ) 
					BETWEEN 20 
					AND 24 
					THEN  '20 to 24'
					WHEN TIMESTAMPDIFF( YEAR, date_birth, date_separated ) 
					...
					AND 59 
					THEN  '55 to 59'
					WHEN TIMESTAMPDIFF( YEAR, date_birth, date_separated ) >=60
					THEN  '> 60'
					END AS agegroup
					FROM  `emp_data_separated` WHERE emp_subgroup NOT LIKE '%Tempo%' 
	...
	{
		// $str = "SELECT COUNT( * ) AS count, termination_reason, DATEDIFF( CURDATE( ) , date_hired ) / 365.25 AS los, 
		$str = "SELECT COUNT( * ) AS count, termination_reason, DATEDIFF( date_separated , date_hired ) / 365.25 AS los, 
					CASE  
						WHEN datediff(date_separated, date_hired) / 365.25  <1 THEN '< 1' 
						WHEN datediff(date_separated, date_hired) / 365.25  >= 1 and datediff(date_separated, date_hired) / 365.25 <= 4 THEN '1 - 4' 
						WHEN datediff(date_separated, date_hired) / 365.25  > 4 and datediff(date_separated, date_hired) / 365.25 <= 9 THEN '5 - 9' 
						WHEN datediff(date_separated, date_hired) / 365.25 > 9 and datediff(date_separated, date_hired) / 365.25 <= 14 THEN '10 - 14' 
						WHEN datediff(date_separated, date_hired) / 365.25 > 14 and datediff(date_separated, date_hired) / 365.25 <= 19 THEN '15 - 19' 
						WHEN datediff(date_separated,date_hired) / 365.25 > 19 and datediff(date_separated, date_hired) / 365.25 <= 24 THEN '20 - 24' 
						WHEN datediff(date_separated, date_hired) / 365.25 > 24 and datediff(date_separated, date_hired) / 365.25 <= 29 THEN '25 - 29' 
						WHEN datediff(date_separated, date_hired) / 365.25 > 29 and datediff(date_separated, date_hired) / 365.25 <= 34 THEN '30 - 34' 
						WHEN datediff(date_separated, date_hired) / 365.25 > 34 and datediff(date_separated, date_hired) / 365.25 <= 39 THEN '35 - 39' 
						WHEN datediff(date_separated, date_hired) / 365.25 > 39 THEN '> 40' END AS los_group 
	...
	{
		$str = "SELECT DISTINCT termination_reason FROM emp_data_separated WHERE emp_subgroup NOT LIKE '%Tempo%'  "; 
		$query = $this->db->query($str); 
		$num_rows = $query->num_rows();
		if($num_rows > 0)
		{
	...
	{
		$str = "SELECT emp_subgroup, COUNT( * ) AS count, SUM( TIMESTAMPDIFF( YEAR, date_birth, CURDATE( ) ) ) / COUNT( * ) AS average FROM emp_data GROUP BY emp_subgroup ";
		$query = $this->db->query($str); 
		$num_rows = $query->num_rows();
		if($num_rows > 0)
		{
...
public function get_average_per_gender()
	{
		$str = "SELECT sex, COUNT( * ) AS count, SUM( TIMESTAMPDIFF( YEAR, date_birth, CURDATE( ) ) ) / COUNT( * ) AS average FROM emp_data GROUP BY sex ";
		$query = $this->db->query($str); 
		$num_rows = $query->num_rows();
		if($num_rows > 0)
		{
...
public function get_average_per_pers_area()
	{
		$str = "SELECT personnel_area, COUNT( * ) AS count, SUM( TIMESTAMPDIFF( YEAR, date_birth, CURDATE( ) ) ) / COUNT( * ) AS average FROM emp_data GROUP BY personnel_area ";
		$query = $this->db->query($str); 
		$num_rows = $query->num_rows();
		if($num_rows > 0)
		{
