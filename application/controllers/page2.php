...
{	
	public function dist_separtion()
	{
		...
		// $foreach(
		// print_r($data['get_count_per_separation_reason_per_year']);
		$separation_type = array();
		foreach($data['get_count_per_separation_reason_per_year']  as $separation)
		{ 
			array_push($separation_type, $separation->termination_reason);
			
			$separation_data[$separation->termination_reason][$separation->year_separated] = $separation->cnt; 
			// echo $separation->cnt;
			// echo "<br>";
			// echo $separation->termination_reason;
			// echo "<br>";
			// echo $separation->year_separated;
			// echo "<br>";
		}
		
		foreach($data['get_separation_type'] as $sep_type)
		{
			for($i = 2007;$i<= 2016;$i++)
			{
				if(!$separation_data[$sep_type->termination_reason][$i])
				{
					$separation_data[$sep_type->termination_reason][$i] = 0;
				}
				
				$data_per_sep_reason_per_year[trim($sep_type->termination_reason)] .=  $separation_data[$sep_type->termination_reason][$i].",";
			}
		}
		...
		$data['data_per_sep_reason_per_year'] = $data_per_sep_reason_per_year; 
		// echo $data_per_sep_reason['resignation (voluntary)'];
